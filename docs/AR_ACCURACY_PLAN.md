...
- No ARKit session restarts in flight; ground-only recalibration popups.
...
corrections have already failed field tests (see above), and there is **no manual correction** either
— so today, a session started with a bad compass stays wrong for the whole flight.
...
| Flight + ADS-B, unpressurized (GA, low/slow) | ADS-B ownship, DR between reports | **pressure ↔ pressure** (0x0A ↔ target baro) — immune to QNH/temp | high (cockpit iron at session start) | P1.1–P1.3, P2 (frame check), P3 |
| Flight + ADS-B, pressurized (fast/high) | ADS-B ownship | **geometric** (0x0B ↔ `alt_geom`/offset-corrected baro); cabin baro auto-rejected | high | P1.1–P1.3, P2 (auto-detect), P3 |
| Flight + internet, unpressurized | phone GPS + DR | pressure (phone absolute baro ↔ target baro) or geometric — whichever is valid | high | P1.4, P2, P3 |
| Flight + internet, pressurized (airliner window seat) | phone GPS (degraded, gated) + DR | geometric (phone GPS ↔ `alt_geom`/offset) | highest (cabin, no calibration chance) | P1.4, P2, P3 |
...
- **Flight recorder:** ring-buffered CSV log (~1 Hz + on events): phone GPS
  ...
  ADS-B ownship (position, 0x0A, 0x0B, track, gs), every applied correction (declination, trim,
  altitude frame chosen, baro→geom offset), and GDL90 CRC-failure counts. Export via share sheet.
- **Info panel additions:** cabin-vs-GPS altitude delta + pressurization verdict, altitude frame in
  use, baro→geom offset estimate, the three heading candidates (compass / GPS track / AR camera yaw).
...
2. **GDL90 completeness:** parse 0x0B ownship geometric altitude; treat `0xFFF` altitude and invalid
   track-type as *missing* (not 0); read the airborne flag. *(F3, F6)*
...
4. **Staleness-aware GPS gating:** accept a degraded fix (up to ~1 km) when the alternative is a fix
   older than ~10 s; gate altitude by `verticalAccuracy` and lightly smooth it. *(F6)*
5. adsb.lol `"ground"` sentinel → on-ground flag, not 0 ft. *(F6)*
...
### P3 — Azimuth: display honesty, manual trim, and *measured* (not guessed) auto-correction
...
2. **Heading readout & 2D map** *(F5)*: in flight, display/orient by GPS or ADS-B track when moving
   ...
   source in the HUD.
3. **Session-start hygiene:** never (re)start ARKit while heading accuracy is poor *in flight*
   (extend the existing ground-only rules); when the app cold-starts airborne, surface a one-time
   "alignment may be off — use trim" hint.
4. **Manual azimuth trim** — the guaranteed path to "exactly": a two-finger horizontal drag (or ±
   buttons when a target is selected) rotates all targets in azimuth; value shown in the info panel,
   reset button, persisted for the session, logged by P0. A pilot who can see the real aircraft can
   null the residual to < 1° in seconds, in any scenario, with any airplane. (Optional follow-up:
   pitch trim by vertical drag for the sustained-turn gravity-tilt case.)
5. **Log-only candidate estimators** (explicitly *not* applied to placement until flights prove
   ...
   ambiguity that plausibly broke the two CLHeading-based attempts; (b) the trim values users apply
   (ground truth for residual-bias statistics). If the logs show a stable, convergent signal, a
   later build may auto-seed the trim with it — behind a setting, default off.

### P4 — Long-range geometry polish
...
### P5 — Tests & field validation protocol
...
- Ground protocol: stand at a known point, verify overflying ADS-B traffic and 2–3 visible airports
  against the camera image; record trim needed.
- Flight protocol (one page, in `docs/`): what to glance at in the info panel per phase of flight,
  and to export the P0 log after landing. Each phase P1–P4 ships as its own build so a single flight
  can attribute any regression.

### Suggested build order
| Build | Content | Risk |
|---|---|---|
| 1 | P0 + P1 (bugs are strict improvements) | low |
| 2 | P2 frame-aware vertical | medium — new decision logic, but fully logged |
| 3 | P3.2–P3.4 heading display + trim | low (trim is user-controlled) |
| 4 | P4 + P3.5 auto-seed *only if* log data supports it | gated on evidence |
...
- Fixing ARKit's internal VIO drift — closed system; we mitigate (no restarts, trim, honest status)
  and measure (P0) instead.
...
- **Any scenario:** if a residual azimuth bias exists, trim nulls it in one gesture and it stays
  nulled for the rest of the flight (drift < ~2°/30 min in cruise, measured by P0 logs).
